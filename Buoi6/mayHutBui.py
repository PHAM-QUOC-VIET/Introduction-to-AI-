...
Link Github: https://github.com/PHAM-QUOC-VIET/Introduction-to-AI-/blob/main/Buoi6/mayHutBui.py
"""
...
class VacuumWorld:
    ...
    def get_successors_ucs(self, state: State):
        """
        Sinh các trạng thái kế tiếp dành riêng cho UCS.
        Chi phí = MOVE_COST + (CLEAN_COST nếu ô hiện tại có bụi)
        Đồng thời, làm sạch ô hiện tại (A) trước khi di chuyển.
        """
        x, y = state.robot_pos
        directions = [(-1, 0, "Up"), (1, 0, "Down"), (0, -1, "Left"), (0, 1, "Right")]
        successors = []
        ...
        for dx, dy, action in directions:
            ...
            # Xác định xem ô hiện tại có bẩn không
            current_dirty = (state.robot_pos in state.dirty_cells)

            # Tạo tập bẩn mới: nếu ô hiện tại bẩn thì làm sạch nó
            new_dirty = set(state.dirty_cells)
            if current_dirty:
                new_dirty.remove(state.robot_pos)

            new_state = State(new_pos, frozenset(new_dirty))

            # Tính chi phí 
            cost = move_cost + (clean_cost if current_dirty else 0)
...
class VacuumGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Button(left, text="IDS_1", width=22, 
                  command=lambda: self.start_search("IDS_1")).pack(pady=6, fill=tk.X)
        ttk.Button(left, text="UCS", width=22, 
                  command=lambda: self.start_search("UCS")).pack(pady=6, fill=tk.X)
    ...
    def ucs(self, start_node: Node):
        ...
        reached = {start_node.state: start_node.path_cost}

        self.queue_text.delete(1.0, tk.END)
        self.queue_text.insert(tk.END, "=== UNIFORM COST SEARCH (UCS) ===\n")
        self.queue_text.insert(tk.END, f"Chi phí di chuyển = 1 | Hút bụi = 2\n\n")
        self.root.update()
        ...
        while frontier:
            ...
            self.update_log(current, is_new_step=True)
            self.root.update()
            time.sleep(0.3)
            ...
            if len(current.state.dirty_cells) == 0:
                ...
                self.queue_text.insert(tk.END, f"\nUCS HOÀN THÀNH!\nTổng path cost = {current.path_cost}\n")
                print(f"UCS HOÀN THÀNH - Total cost = {current.path_cost}")
                return current

            # Mở rộng các node con
            for next_state, action, step_cost in self.world.get_successors_ucs(current.state):
                new_cost = current.path_cost + step_cost
        ...
        self.queue_text.insert(tk.END, "\nUCS: Không tìm thấy giải pháp\n")
        return None


    def ids1(self, start_node: Node):
        depth = 0
        max_depth = 20
        ...
        while depth <= max_depth:
            self.queue_text.delete(1.0, tk.END)
            self.path_text.delete(1.0, tk.END)
            self.queue_text.insert(tk.END, f"🔍 IDS - Đang thử Depth = {depth}\n\n")
            self.root.update()

            result = self.recursive_dls1(start_node, depth)
            
            if result not in ("cutoff", "failure"):
                self.queue_text.insert(tk.END, f"\nTÌM THẤY GOAL ở depth = {depth} | Tổng path_cost = {result.path_cost}\n")
                return result
    ...
    def recursive_dls1(self, node: Node, limit: int):
        if len(node.state.dirty_cells) == 0:
            self.draw_grid(node.state.dirty_cells, node.state.robot_pos)
            self.print_path(node)
            self.queue_text.insert(tk.END, f"\nGOAL FOUND! Tổng path_cost: {node.path_cost}\n")
            return node
        ...
        for new_state, action in self.world.get_successors(node.state):
            child = Node(
                state=new_state,
                parent=node,
                action=action,
            ...
            self.update_log(child)
            self.draw_grid(new_state.dirty_cells, new_state.robot_pos)
            self.root.update()
            time.sleep(0.5)

            result = self.recursive_dls1(child, limit - 1)

            if result == "cutoff":
                cutoff_occurred = True
            elif result not in ("failure", "cutoff"):
                return result
